...
async function renderSidebar() {
  ...
  container.innerHTML = categories.map((cat, catIdx) => {
    ...
    const isFullyCompleted = completed === topics.length && topics.length > 0;
    ...
    return `
    <div class="cat-accordion-item ${isActiveCat ? 'active-cat' : ''} ${isFullyCompleted ? 'fully-completed' : ''}">
...
function renderLesson() {
  ...
  const mistakesHtml = currentLesson.common_mistakes && currentLesson.common_mistakes.length > 0
    ? `<div class="lesson-section">
        <div class="lesson-section-label"> COMMON MISTAKES</div>
