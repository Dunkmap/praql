...
function verifySignature(rawBody, signatureHeader, secret) {
  ...
  try {
    ...
    // Buffers are different lengths → definitely not equal
    return false;
  }
}
...
module.exports = async function handler(req, res) {
  ...
  // Vercel automatically parses JSON bodies — we need the RAW body for
  // signature verification. Vercel exposes req.body as parsed JSON when
  // Content-Type is application/json.  We need to reconstruct the raw
  // string to verify the HMAC.
  //
  // For Vercel: set "bodyParser: false" in route config, or reconstruct
  // from the parsed body.  Reconstructing JSON.stringify(req.body) is
  // NOT safe because key order / whitespace may differ.
  //
  // Best approach: read the raw body ourselves.
  let rawBody;
  ...
  if (typeof req.body === 'string') {
    ...
    // Vercel already parsed it — we need to read raw stream instead
    // This shouldn't happen with bodyParser disabled, but as a fallback:
  ...
  console.log(`[Webhook] ✓ Verified event: ${event.event_type}`, {
    event_id: event.event_id,
    occurred_at: event.occurred_at,
  });

  // Future: persist subscription data to a database here.
  // For now, we rely on the Paddle API for real-time lookups
  // via /api/verify-subscription.
