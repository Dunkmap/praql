    ...
    for (const result of results) {
      ...
      html += `
        <div class="result-container" style="margin-top:24px; animation: slideUp 0.4s ease;">
          ...
          <div class="result-table-wrapper" style="border: 1px solid var(--border-color); border-radius: 16px; overflow: hidden; background: var(--bg-card); box-shadow: var(--shadow-md);">
            <table class="result-table" style="width:100%; border-collapse: collapse;">
              <thead>
                <tr style="background: var(--bg-tertiary);">`;
      
      for (const col of result.columns) {
        html += `<th style="padding:16px; text-align:left; font-weight:800; font-size:0.85rem; color:var(--accent-pink); border-bottom: 1px solid var(--border-color); text-transform: uppercase; letter-spacing: 1px;">${escapeHtml(col)}</th>`;
      }
